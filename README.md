...
> 之所以输出多余的字段是考虑到方便后续的人工二次审查，但是这玩意实在太多了，我是顶不住了，就只简单检查了下。如果你想自己审一下可以对照着这些字段自己检查校正。不过需要注意的是 `is_correct` 字段需要看一下。如果该字段输出无漏洞，不一定是模型没检测到，可能只是模型没输出有效格式的 CWE 编号导致正则表达式匹配失败。如果出现这种情况则需要我们手工检查一下。

**最后注意，毕竟是调的 API，程序也无法保证所有输出都是满足要求的，有时候也会出现生成失败或者输出英文等情况（可能还会出现比如输出和提取出的结果/正确答案与真实标签不一致的情况），这些数据需要我们手动处理一下。**
